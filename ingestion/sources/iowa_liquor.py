...
def fetch_rows(start_date: str | None = None, end_date: str | None = None) -> list[dict]:
    """
    Fetch a small sample of Iowa liquor data and normalize it into
    the RAW_IOWA_LIQUOR table shape expected by the ingestion runner.
    Optionally filter the API request by start and end date.
    """
    params = {"$limit": 5}
        ...
        rows.append(
            {
                ...
                "store_location": str(record.get("store_location")) if record.get("store_location") is not None else None,
